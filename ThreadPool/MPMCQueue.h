...
/// CRTP specialization of MPMCQueueBase
        ...
        public:
            ...
            explicit MPMCQueueBase(size_t queueCapacity)
                ...
                assert(
                        alignof(MPMCQueue<T, Atom>) >= folly::hardware_destructive_interference_size);
                assert(
                        static_cast<uint8_t *>(static_cast<void *>(&popTicket_)) -
                        static_cast<uint8_t *>(static_cast<void *>(&pushTicket_)) >=
                        static_cast<ptrdiff_t>(folly::hardware_destructive_interference_size));
            }
        ...
        protected:
            enum {
                ...
                /// each end
                kSlotPadding =
                (folly::hardware_destructive_interference_size - 1) / sizeof(Slot) + 1
            };

            /// The maximum number of items in the queue at once
            // 队列的最大容量
            alignas(folly::hardware_destructive_interference_size) size_t capacity_;
            ...
            // 写入计数器
            alignas(folly::hardware_destructive_interference_size) Atom<uint64_t> pushTicket_;

            /// Dequeuers get tickets from here
            // 读取计数器
            alignas(folly::hardware_destructive_interference_size) Atom<uint64_t> popTicket_;
            ...
            // 3. 它的值会动态调整,以实现自适应自旋。
            alignas(
            folly::hardware_destructive_interference_size) Atom<uint32_t> pushSpinCutoff_;

            /// The adaptive spin cutoff when the queue is empty on dequeue
            alignas(folly::hardware_destructive_interference_size) Atom<uint32_t> popSpinCutoff_;

            /// Alignment doesn't prevent false sharing at the end of the struct,
            /// so fill out the last cache line
            char pad_[folly::hardware_destructive_interference_size - sizeof(Atom<uint32_t>)];
...
/// used by SingleElementQueue are doubled inside the TurnSequencer
        ...
        struct SingleElementQueue {
            ...
            bool tryWaitForEnqueueTurnUntil(
                ...
                return sequencer_.tryWaitForTurn(
                       ...
                       folly::detail::TurnSequencer<Atom>::TryWaitResult::TIMEDOUT;
            }
        ...
        private:
            ...
            folly::aligned_storage_for_t<T> contents_;
            ...
            folly::detail::TurnSequencer<Atom> sequencer_;
